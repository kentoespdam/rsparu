...
class PasienModel extends CI_Model {
	...
	public function findNorm($noRm) {
		...
		if ($q->num_rows() > 0) {
			...
			// $res = $this->ResponseModel->res204(['data' => $arrData, "count" => $q->num_rows()]);
			// Jika data tidak ditemukan di database lokal, coba cari di Kominfo
			$kominfoResult = $this->cariDataKominfo($noRm);

			if ($kominfoResult) {
				$res = $this->ResponseModel->res200(['data' => $kominfoResult, "count" => count($kominfoResult)]);
			} else {
				$res = $this->ResponseModel->res204(['data' => $arrData, "count" => $q->num_rows()]);
			}
		}
	...
	private function cariDataKominfo($noRm) {
    // Lakukan request ke layanan Kominfo
    ...
    // Proses dan kembalikan data hasil pencarian dari Kominfo
    $jsonData = json_decode($response, true); // Set ke true agar dihasilkan sebagai array asosiatif
    return $jsonData;
	}
}