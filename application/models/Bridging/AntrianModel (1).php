...
//                $Klinik = $Klinik;
    ...
    public function do_simpan_antrianpoli($Klinik, $No_Pasien, $Nama, $kelurahan, $kecamatan, $kabupaten, $NOANTRI, $umur, $jkel, $status) {
        ...
        $cek_exist = $this->cek_exist($No_Pasien, $Tanggal);
...
//            print_r($sql_u);
            $this->ano->trans_start();
            $this->ano->update("tbantrianpoli", $sql_u, ["No_Pasien" => $No_Pasien, "tbantrianpoli.Tanggal" => $cek_exist->result()[0]->Tanggal]);
            $this->ano->trans_complete();
...
//
    ...
    private function cek_exist($No_Pasien, $Tanggal) {
        $Tanggal = date('Y-m-d', strtotime($Tanggal));
        $sql = "SELECT * FROM `tbantrianpoli` WHERE tbantrianpoli.No_Pasien = '$No_Pasien' AND DATE_FORMAT( tbantrianpoli.Tanggal, '%Y-%m-%d' ) = '$Tanggal'";
